...
class TitleScene {
  ...
  draw() {
    ...
    Gfx.draw("warrior", 60, 150 + bob, { scale: 2 });
    Gfx.draw("mage", 150, 152 - bob, { scale: 2 });
    Gfx.draw("cleric", 200, 150 + bob, { scale: 2 });
    Gfx.draw("rogue", 250, 152 - bob, { scale: 2 });

    Gfx.text("ARCANA DUNGEON", 92, 46, 2, 14);
    Gfx.text("アルカナダンジョン", 88, 70, 3, 16);
    Gfx.text("〜ジョブと なかまの めいきゅうたん〜", 60, 96, 2, 11);
...
function bootGame() {
  ...
  function loop(now) {
    ...
    try {
      ...
      errFrames++;
      if (errFrames <= 3) console.error("[アルカナダンジョン] フレームエラー:", e);
