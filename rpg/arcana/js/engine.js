...
const GBC_PALS = {
  ...
  gold:    ["#f8f0c8", "#f0c848", "#b07818", "#402808"], // 宝・アルカナ
  holy:    ["#f8f0e8", "#f0b0c8", "#a05880", "#381028"], // 聖・回復系ジョブ
  arcane:  ["#e8e0f0", "#9080c0", "#504088", "#181030"], // 魔・とくぎ系ジョブ
};
const GBC_TILE = {
...
const GBC_CHAR = {
  warrior: "earth", mage: "arcane", cleric: "holy", rogue: "night",
  ...
  sprite(name, variant) {
    ...
    for (let y = 0; y < rows.length; y++) {
      ...
      for (let x = 0; x < row.length; x++) {
        ...
        else if (variant === "light") pi = Math.max(0, pi - 1);
...
const G = {
  ...
  // ---------- パーティ/ジョブ/スキル ----------
  // h.cls = げんざいのジョブID。h.jobSp = {ジョブID: SP} でジョブごとの熟練度。
  // h.skillLib = これまでに どれかのジョブで おぼえた ぜんスキルID (くみかえの もとネタ)。
  // h.subskills = べつジョブから もちこんだ サブスキル (さいだい3つ)。
  makeHero(id, lv) {
    const d = DATA.heroes[id];
    const jd = DATA.jobs[d.defaultJob];
    const h = {
      id, name: d.name, cls: d.defaultJob,
      lv, exp: this.expTotalFor(lv),
      weapon: jd.weapon || null, armor: jd.armor || null, acc: null,
      jobSp: {}, skillLib: [], subskills: [],
      poison: false, paralyze: false,
    };
    this.applyStats(h);
  ...
  applyStats(h) {
    const d = DATA.jobs[h.cls];
    const n = h.lv - 1;
    h.maxhp = Math.round(d.base.hp + d.growth.hp * n);
    h.maxmp = Math.round(d.base.mp + d.growth.mp * n);
    h.str = Math.round(d.base.str + d.growth.str * n);
    h.agi = Math.round(d.base.agi + d.growth.agi * n);
    h.vit = Math.round(d.base.vit + d.growth.vit * n);
    h.int = Math.round(d.base.int + d.growth.int * n);
    // 果実などによる 恒久ボーナス
    h.maxhp += h.bonusHp || 0;
    h.maxmp += h.bonusMp || 0;
    h.str += h.bonusStr || 0;
  ...
  // ジョブを きりかえる。げんざいの HP/MPは 全回復 (きりかえ直後に こまらないように)。
  // そうびの ぶき/よろいは そのまま (ジョブとは べつかんりの そうび)。
  changeJob(h, jobId) {
    h.cls = jobId;
    this.applyStats(h);
    h.hp = h.maxhp; h.mp = h.maxmp;
  ...
  // かくとくEXP -> レベルアップしょり。メッセージれつをかえす (スキルは SPで べつにおぼえる)
  addExp(h, amount) {
    const msgs = [];
    h.exp += amount;
    ...
    while (h.lv < this.MAX_LV && h.exp >= this.expTotalFor(h.lv + 1)) {
      ...
      const gains = [["HP", h.maxhp - bs.hp], ["MP", h.maxmp - bs.mp],
        ["力", h.str - bs.str], ["速さ", h.agi - bs.agi],
  ...
  // SP (熟練ポイント) → げんざいのジョブの スキルを かくとく。メッセージれつをかえす。
  addSp(h, amount) {
    const msgs = [];
    const jobId = h.cls;
    h.jobSp[jobId] = (h.jobSp[jobId] || 0) + amount;
    const learn = DATA.jobs[jobId].learn || {};
    const thresholds = Object.keys(learn).map(Number).sort((a, b) => a - b);
    for (const sp of thresholds) {
      const skillId = learn[sp];
      if (h.jobSp[jobId] >= sp && !h.skillLib.includes(skillId)) {
        h.skillLib.push(skillId);
        msgs.push(`${h.name}は 「${DATA.skills[skillId].name}」を おぼえた!`);
      }
    }
    return msgs;
  },

  // げんざいのジョブが つかえる スキル (ジョブしょゆう + サブスキル)
  jobSkillsOf(h) {
    const learn = DATA.jobs[h.cls].learn || {};
    return Object.values(learn).filter((id) => h.skillLib.includes(id));
  },
  usableSkillsOf(h) {
    const job = this.jobSkillsOf(h);
    const sub = (h.subskills || []).filter((id) => !job.includes(id) && h.skillLib.includes(id));
    return job.concat(sub);
  },
  ...
  atkOf(h) { return Math.round(this.strOf(h) + (DATA.items[h.weapon]?.atk || 0)); },
  defOf(h) {
  ...
  calcHeal(h, skill) { return Math.round(skill.pow * (1 + this.intOf(h) / 16)); },
  ...
  currentChapter(flags) {
    ...
    if (f.clearedRuins) return "迷宮バギンの踏破者";
    if (f.party4) return "冒険者パーティ";
    return "旅のはじまり";
  },
  ...
  importCode(text, slot) {
    try {
      ...
      if (!s || !Array.isArray(s.party)) return false;
  ...
  slotInfo(n) {
    try {
      ...
      const top = s.party[0];
      return {
        name: top ? top.name : "たびのはじまり", lv: top ? top.lv : 1,
...
function runScript(ops, onDone) {
  ...
  const next = () => {
    while (i < ops.length) {
      ...
      if (op.cond) {
        // flag / item(しょじひん) / all(ぜんフラグ) で ぶんき
        let pass;
      ...
      if (op.join) {
        // なかまを パーティへ: { join: "キャラID" }
        if (!G.state.party.some((h) => h.id === op.join) && G.state.party.length < 4) {
          const lv = G.state.party.length ? G.state.party[0].lv : 1;
      ...
      if (op.battle) {
        // { wild: [{id,lv}, ...] } または { trainer: "トレーナーID" }
        G.push(new BattleScene(Object.assign({}, op.battle, { onWin: next })));
        return;
      }
      if (op.warp) {
        const w = op.warp;
            ...
            if (sel === 0) {
              if (G.state.gold < price) {
                ...
                G.fade(() => {
                  G.state.party.forEach((h) => {
                    h.hp = h.maxhp; h.mp = h.maxmp;
                    h.poison = false; h.paralyze = false;
                  });
                  AudioSys.sfx("heal");
