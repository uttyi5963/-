...
class FieldScene {
  ...
  checkEncounter() {
    ...
    if (!table) return;
    if (G.aliveParty().length === 0) return; // 戦えるなかまがいなければ出ない
    if (Math.random() < table.rate) {
      // おもみつき抽選: mons = [[敵ID, 最小Lv, 最大Lv, おもみ], ...]
      const total = table.mons.reduce((s, e) => s + (e[3] || 1), 0);
      let r = Math.random() * total;
      let pick = table.mons[0];
      for (const e of table.mons) { r -= e[3] || 1; if (r <= 0) { pick = e; break; } }
      const lv = pick[1] + Math.floor(Math.random() * (pick[2] - pick[1] + 1));
      // グループせいせい: table.count (なければ1) たいまで まとめて出現
      const n = 1 + Math.floor(Math.random() * (table.count || 1));
      const group = [];
      for (let k = 0; k < n; k++) group.push({ id: pick[0], lv });
      AudioSys.sfx("encounter");
      G.push(new BattleScene({ wild: group }));
    }
  }
