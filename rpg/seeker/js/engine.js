// ============================================================
// モンスターシーカー - エンジン (クリスタルナイツ エンジンを継承)
// ============================================================
...
const G = {
  ...
  makeMon(id, lv) {
    ...
    this.monStats(m);
    // レベルまでの しゅうとくずみの技 (さいだい4つ・あたらしい技を優先)
    const learned = Object.entries(sp.learn)
      .filter(([l]) => lv >= +l)
      .sort((a, b) => +a[0] - +b[0])
      .map(([, mv]) => mv);
    m.moves = learned.slice(-4);
    if (m.moves.length === 0) m.moves = ["tackle"]; // まんいちの安全網
    m.hp = m.maxhp;
    return m;
  },
  ...
  monStats(m) {
    ...
    const n = m.lv - 1;
    m.maxhp = sp.base.hp + Math.round(sp.growth.hp * n);
    m.atk = sp.base.atk + Math.round(sp.growth.atk * n);
    m.def = sp.base.def + Math.round(sp.growth.def * n);
    m.spd = sp.base.spd + Math.round(sp.growth.spd * n);
  },

  speciesOf(m) { return DATA.species[m.id]; },

  // 経験値 → レベルアップ・技しゅうとく・進化。メッセージれつをかえす
  addMonExp(m, amount) {
    const msgs = [];
    m.exp += amount;
    const capExp = this.expTotalFor(this.MAX_LV);
    if (m.exp > capExp) m.exp = capExp;
    while (m.lv < this.MAX_LV && m.exp >= this.expTotalFor(m.lv + 1)) {
      m.lv++;
      const before = { hp: m.maxhp, atk: m.atk, def: m.def, spd: m.spd };
      this.monStats(m);
      m.hp = Math.min(m.maxhp, m.hp + (m.maxhp - before.hp));
      const gains = [["HP", m.maxhp - before.hp], ["攻", m.atk - before.atk],
        ["防", m.def - before.def], ["速", m.spd - before.spd]]
        .filter(([, v]) => v > 0).map(([k, v]) => `${k}+${v}`).join(" ");
      msgs.push(`${m.name}は レベル${m.lv}に あがった!` + (gains ? `\n${gains}` : ""));
      // 技しゅうとく (4つをこえたら いちばん古い技と いれかえ)
      const learn = DATA.species[m.id].learn[m.lv];
      if (learn && !m.moves.includes(learn)) {
        if (m.moves.length >= 4) {
          const old = m.moves.shift();
          msgs.push(`${m.name}は ${DATA.moves[old].name}を わすれて\n${DATA.moves[learn].name}を おぼえた!`);
        } else {
          msgs.push(`${m.name}は ${DATA.moves[learn].name}を おぼえた!`);
        }
        m.moves.push(learn);
      }
      // 進化
      const ev = DATA.species[m.id].evolve;
      if (ev && m.lv >= ev.lv) {
        const oldName = m.name;
        m.id = ev.to;
        m.name = DATA.species[ev.to].name;
        this.monStats(m);
        m.hp = m.maxhp;
        this.recordSeen(m.id);
        this.recordCaught(m.id);
        msgs.push(`なんと!? ${oldName}は\n${m.name}に しんかした!!`);
      }
    }
    return msgs;
  },
  ...
  currentChapter(flags) {
    ...
    if (f.badge1) return "認定シーカー";
    if (f.starter) return "見習いシーカー";
    return "旅立ちの朝";
  },
  ...
  recordCaught(id) { this.bestiaryEntry(id).caught = (this.bestiaryEntry(id).caught || 0) + 1; },
  killsOf(id) {
  ...
  slotInfo(n) {
    try {
      ...
      return {
        ...
        trueClear: !!(s.flags && s.flags.badge1),
...
function runScript(ops, onDone) {
  ...
  const next = () => {
    while (i < ops.length) {
      ...
      if (op.starter) {
        // さいしょの相棒を パーティへ
        const m = G.makeMon(op.starter, 5);
        G.state.party.push(m);
        G.recordSeen(op.starter);
        G.recordCaught(op.starter);
        G.setFlag("starter", 1);
        AudioSys.sfx("levelup");
        continue;
      }
